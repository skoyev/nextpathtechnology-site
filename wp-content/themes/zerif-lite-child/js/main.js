jQuery( document ).ready(function($) {
    ...
    const focus = $('#focus')
    if(focus){
        focus.css({'background':'#f0f0f0', 'padding-top':'25px'})
    }

    const aboutus = $('#aboutus')
    if(aboutus){
        aboutus.css({'padding-top':'25px'})
    }    
	...
	contactUsEl += '<span id="mn_a" style="color: #ffffff;font-style:italic">' + getRandomValue() + '</span><span style="color: #ffffff;font-style:italic"> + </span><span id="mn_b" style="color: #ffffff;font-style:italic">' + getRandomValue() + '</span><span style="color: #ffffff;font-style:italic"> = </span><input type="text" name="mn_answer" id="mn_answer" value="" maxlength="4" style="border-radius: 3px; width:60px; margin:0 5px;" required="required"><small style="color: #ffffff;"></small>';
    ...
    $('#mncontactfrm-home').submit(function( event ) {
	    ...
	    var data = { 'action':'get_data', 
  		...
  		$.post(ajaxUrl, data , function(result) {
           ...
           $('#email-msg').delay(5000).html('<span style="font-size:18px;"></span>');
           $('#email-msg').delay(5000).fadeIn('slow');
		});
